...
public class CacheVenusApplicationDaoImpl implements CacheApplicationDAO {
	...
	public void init() {
		GlobalScheduler.getInstance().scheduleAtFixedRate(new LoadCacheApplicationsRunnable(), 1, 1, TimeUnit.SECONDS);
	}
		...
		public void run() {
			try {
				...
				LogUtils.DEFAULT_LOG.info(
						"LoadCacheApplicationsRunnable consumerTime=>{},cacheCodeApplicationMap size=>{}", consumerTime,
						cacheCodeApplicationMap.size());
			} catch (Throwable e) {
