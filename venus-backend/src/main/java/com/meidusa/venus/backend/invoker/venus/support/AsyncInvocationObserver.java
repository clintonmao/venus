package com.meidusa.venus.backend.invoker.venus.support;

import com.meidusa.venus.backend.invoker.venus.EndpointInvocation;
import com.meidusa.venus.backend.support.RequestContext;

public class AsyncInvocationObserver implements InvocationObserver {

    @Override
    public void afterInvoke(EndpointInvocation invocation, RequestContext context) {

    }

    @Override
    public void beforeInvoke(EndpointInvocation invocation, RequestContext context) {

    }

}
